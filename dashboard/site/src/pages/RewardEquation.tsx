...
export default function RewardEquation() {
  ...
  const rewardColor = reward.total >= 0 ? "text-green-600" : "text-red-600";
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-6">
      ...
      {/* Equation display */}
      <div className="bg-gray-50 rounded-lg p-5 mb-8 font-mono text-sm leading-relaxed border border-gray-200">
        <div className="text-center text-base mb-3 font-semibold text-gray-700">
        ...
        <div className="grid grid-cols-4 gap-3 text-xs text-gray-500">
          <div className="text-center">
            <span className="font-semibold text-gray-700">{reward.term_eat.toFixed(4)}</span>
            <br />food term
          </div>
          <div className="text-center">
            <span className="font-semibold text-gray-700">{reward.term_act.toFixed(4)}</span>
            <br />motor term
          </div>
          <div className="text-center">
            <span className="font-semibold text-gray-700">{reward.term_prey.toFixed(4)}</span>
            <br />prey proximity
          </div>
          <div className="text-center">
            <span className="font-semibold text-gray-700">{reward.term_pred.toFixed(4)}</span>
      ...
      <div className="text-center mb-8">
        <div className="text-sm text-gray-500 uppercase tracking-wide mb-1">Computed Reward</div>
      ...
      <div className="grid grid-cols-2 gap-8">
        {/* Genome weights */}
        <div>
          <h2 className="text-lg font-semibold mb-4 text-gray-800">Genome Weights (evolved)</h2>
        ...
        {/* Stimulus inputs */}
        <div>
          <h2 className="text-lg font-semibold mb-4 text-gray-800">Stimulus Inputs (from environment)</h2>

          <div className="mb-5">
            <div className="flex justify-between text-sm mb-1">
              <span className="font-medium">n_eaten</span>
              <span className="text-gray-500">{nEaten}</span>
            </div>
            <div className="flex gap-3">
              <button
                className={`px-4 py-1.5 rounded text-sm font-medium transition ${nEaten === 0 ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                onClick={() => setNEaten(0)}
              >0 (no food)</button>
              <button
                className={`px-4 py-1.5 rounded text-sm font-medium transition ${nEaten === 1 ? "bg-green-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
                onClick={() => setNEaten(1)}
              >1 (ate food)</button>
            </div>
          </div>
      ...
      <div className={`mt-8 p-5 rounded-lg border-2 ${
        ...
        <h3 className="font-semibold text-gray-800 mb-2">Key Insight: The Emergence of Fear</h3>
        <p className="text-sm text-gray-600 leading-relaxed">
          ...
          <span className={reward.term_pred < 0 ? "text-red-600 font-semibold" : "text-gray-700"}>
      ...
      {/* Preset scenarios */}
      <div className="mt-8">
        <h3 className="font-semibold text-gray-800 mb-3">Preset Scenarios</h3>
...
function WeightSlider({ label, sublabel, value, onChange }: {
  ...
  return (
    <div className="mb-5">
      <div className="flex justify-between text-sm mb-1">
        <span className="font-medium">{label} <span className="text-gray-400 text-xs">({sublabel})</span></span>
...
function PresetButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="px-3 py-1.5 rounded border border-gray-300 text-sm text-gray-700 hover:bg-gray-100 transition"
    >
      {label}
    </button>
  );
