...
const SEGMENT_COLORS: Record<string, { bg: string; border: string; text: string; label: string }> = {
  proximity_sensors: { bg: "bg-blue-100", border: "border-blue-400", text: "text-blue-800", label: "Proximity" },
  tactile_collision: { bg: "bg-green-100", border: "border-green-400", text: "text-green-800", label: "Tactile" },
  velocity: { bg: "bg-orange-100", border: "border-orange-400", text: "text-orange-800", label: "Velocity" },
  angle: { bg: "bg-purple-100", border: "border-purple-400", text: "text-purple-800", label: "Angle" },
  angular_velocity: { bg: "bg-purple-100", border: "border-purple-300", text: "text-purple-700", label: "Ang. Vel." },
  energy: { bg: "bg-red-100", border: "border-red-400", text: "text-red-800", label: "Energy" },
};
...
export default function ObservationLayout() {
  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-6">
      ...
      {/* Stacked bar */}
      <div className="mb-8">
        <div className="flex rounded-lg overflow-hidden border border-gray-300 h-16">
            ...
            return (
              <button
                ...
                className={`${colors.bg} ${colors.text} flex items-center justify-center text-xs font-medium
                  hover:brightness-95 transition cursor-pointer relative
                  ${isSelected ? `ring-2 ring-offset-1 ${colors.border} ring-blue-500` : ""}`}
            ...
            <tbody>
              {obsData.segments.map((seg) => (
                <tr key={seg.name} className="border-b border-gray-100">
                  ...
                  <td className="py-2 text-gray-600">{seg.description}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
  ...
  return (
    <div className={`rounded-lg p-6 border-2 ${colors.border} ${colors.bg} mb-6`}>
      ...
      {"details" in segment && segment.details && (
        <p className="text-sm text-gray-600 mb-4">{segment.details}</p>
      )}

      {/* Proximity sensors detail */}
...
function ProximityDetail() {
  ...
  const channelColors = ["text-blue-700", "text-red-700", "text-green-700", "text-gray-700"];
  const nSensors = 32;
  const nChannels = 4;
    ...
    <div>
      ...
      <div className="overflow-x-auto mb-4">
        <div className="inline-block">
          ...
          {/* Sensor rows (show first 8 and last 2 for brevity) */}
          {Array.from({ length: 10 }).map((_, rowIdx) => {
            const sensorIdx = rowIdx < 8 ? rowIdx : nSensors - (10 - rowIdx);
            ...
            return (
              <div key={sensorIdx} className="flex items-center gap-0.5 mb-0.5">
                <div className="w-10 text-right text-[9px] text-gray-500 pr-1 font-mono">
                  s{sensorIdx}
                </div>
                {channels.map((ch, chIdx) => {
                  ...
                  return (
                    <div
                      key={ch}
                      className="w-7 h-5 rounded-sm bg-white border border-gray-200 text-[8px] text-center leading-5 text-gray-400"
      ...
      <div className="text-xs bg-white/50 rounded p-3 border border-blue-200">
...
function TactileDetail() {
  ...
  const typeColors = ["bg-blue-200", "bg-red-200", "bg-green-200", "bg-gray-200"];
  const nBins = 18;
        ...
        <div className="inline-block">
          ...
          {types.map((type, typeIdx) => (
            <div key={type} className="flex items-center gap-0.5 mb-0.5">
              <div className="w-18 text-right text-[9px] text-gray-500 pr-1 truncate" style={{ width: "75px" }}>
                {type}
              </div>
              {Array.from({ length: nBins }).map((_, binIdx) => {
                ...
                return (
                  <div
                    key={binIdx}
                    className={`w-5 h-5 rounded-sm ${typeColors[typeIdx]} border border-gray-200 text-[7px] text-center leading-5 text-gray-500`}
