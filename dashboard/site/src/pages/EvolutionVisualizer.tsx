...
// Student's t via ratio of normals: t(df) = Z / sqrt(chi2(df)/df)
...
function DistributionComparison() {
  const data = useMemo(() => {
    ...
    // Gaussian (std=0.4 to match scale)
    ...
    return {
      gaussian: generateHistogram(gaussian, bins, range),
      cauchy: generateHistogram(cauchy, bins, range),
      t2: generateHistogram(t2, bins, range),
      range,
    };
  }, []);

  const maxDensity = Math.max(
    ...data.gaussian.counts,
    ...data.cauchy.counts,
    ...data.t2.counts,
  );
  ...
  function xScale(v: number) {
    return margin.left + ((v - data.range[0]) / (data.range[1] - data.range[0])) * plotW;
  }
  function yScale(v: number) {
    return margin.top + plotH - (v / (maxDensity * 1.1)) * plotH;
  }

  function makePath(hist: { centers: number[]; counts: number[] }) {
    return hist.centers
      .map((c, i) => `${i === 0 ? "M" : "L"} ${xScale(c).toFixed(1)} ${yScale(hist.counts[i]).toFixed(1)}`)
      .join(" ");
  }
  ...
  return (
    <div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-3xl bg-white border border-gray-200 rounded-lg">
        {/* Axes */}
        ...
        {/* X axis labels */}
        {[-4, -2, 0, 2, 4].map((v) => (
          <text key={v} x={xScale(v)} y={margin.top + plotH + 20} textAnchor="middle"
            className="text-[10px] fill-gray-500">{v}</text>
        ))}
        <text x={margin.left + plotW / 2} y={H - 5} textAnchor="middle"
          className="text-[11px] fill-gray-600">Mutation magnitude</text>
        ...
        {/* Zero line */}
        <line x1={xScale(0)} y1={margin.top} x2={xScale(0)} y2={margin.top + plotH}
          stroke="#ddd" strokeWidth={1} strokeDasharray="3 3" />

        {/* Legend */}
        <rect x={W - 170} y={margin.top + 5} width={150} height={70} fill="white" stroke="#eee" rx={4} />
        <line x1={W - 160} y1={margin.top + 22} x2={W - 140} y2={margin.top + 22} stroke="#4e79a7" strokeWidth={2} />
        <text x={W - 135} y={margin.top + 26} className="text-[10px] fill-gray-700">Gaussian (s=0.4)</text>
        <line x1={W - 160} y1={margin.top + 42} x2={W - 140} y2={margin.top + 42} stroke="#e15759" strokeWidth={2.5} />
        <text x={W - 135} y={margin.top + 46} className="text-[10px] fill-gray-700">t(df=2, s=0.4) — K&D 2025</text>
        <line x1={W - 160} y1={margin.top + 62} x2={W - 140} y2={margin.top + 62} stroke="#76b7b2" strokeWidth={1.5} strokeDasharray="4 2" />
        <text x={W - 135} y={margin.top + 66} className="text-[10px] fill-gray-700">Cauchy (t df=1, s=0.4)</text>
      </svg>
    </div>
  );
}
...
function MutationDemo() {
  ...
  function renderBar(value: number, color: string) {
    ...
    return (
      <div className="relative h-7 bg-gray-100 rounded" style={{ width: barW }}>
        <div className="absolute top-0 bottom-0 w-px bg-gray-300" style={{ left: barW / 2 }} />
        <div
          className={`absolute top-0.5 bottom-0.5 rounded ${color}`}
          style={{ left: x, width: Math.max(w, 2) }}
        />
        <span className="absolute right-1 top-0.5 text-[10px] text-gray-500 font-mono">
          {value.toFixed(2)}
        </span>
      </div>
    );
  ...
  return (
    <div>
      ...
      <p className="text-sm text-gray-500 mb-4">
        Click "Mutate" to add t(df=2, scale=0.4) noise to the parent genome. Repeat to see how the
        child genome drifts. Clipped to [-100, 100].
      </p>

      <div className="grid grid-cols-3 gap-6">
        {/* Parent */}
        <div>
          <div className="text-sm font-semibold mb-2 text-gray-600">Parent Genome</div>
          {WEIGHT_NAMES.map((wn, i) => (
            <div key={wn} className="mb-2">
              <div className="text-xs text-gray-500 mb-0.5">{wn}</div>
              {renderBar(parentGenome[i], "bg-blue-400")}
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col items-center justify-center">
          <button
            onClick={mutate}
            className="px-5 py-2.5 bg-red-500 text-white rounded-lg font-medium hover:bg-red-600 transition mb-3"
          >
            Mutate
          </button>
          {mutationNoise && (
            <div>
              <div className="text-xs text-gray-400 text-center mb-1">noise added:</div>
              {WEIGHT_NAMES.map((wn, i) => (
                <div key={wn} className="text-xs font-mono text-gray-500 text-center">
        ...
        {/* Child */}
        <div>
          <div className="text-sm font-semibold mb-2 text-gray-600">Child Genome</div>
          {childGenome ? (
            WEIGHT_NAMES.map((wn, i) => (
              <div key={wn} className="mb-2">
                <div className="text-xs text-gray-500 mb-0.5">{wn}</div>
                {renderBar(childGenome[i], "bg-green-400")}
              </div>
            ))
          ) : (
            <div className="text-sm text-gray-400 mt-8 text-center">Click Mutate</div>
          )}
          {childGenome && (
            <button
              onClick={() => { setParentGenome(childGenome); setChildGenome(null); setMutationNoise(null); }}
  ...
  return (
    <div className="bg-amber-50 border border-amber-200 rounded-lg p-5">
      ...
      <div className="text-sm text-gray-700 space-y-2">
        <p>
          <strong>Gaussian mutation</strong> (the standard approach) produces small, incremental changes.
          Most mutations move the reward weights by &lt;1 standard deviation. The population explores
          reward space slowly and can get stuck in local optima.
        </p>
        <p>
          <strong>Student's t(df=2)</strong> mutation (K&D 2025) has much heavier tails. While most mutations
          are still small, there's a meaningful probability of large jumps — mutations 5-10x larger than
          typical. These occasional large jumps:
        </p>
        ...
        <p>
          <strong>Cauchy (t df=1)</strong> has even heavier tails, but K&D found t(df=2) works better in practice.
          The 2024 paper used Cauchy with scale=0.02; the 2025 paper switched to t(df=2) with scale=0.4.
          The larger scale + lighter (but still heavy) tails produce more reliable evolutionary dynamics.
        </p>
      </div>
    </div>
  );
