...
**Prey fear evolves, then erodes.** Linear w_pred (prey) trajectory: ~0 at start → drifted to **-3.9 at ≈810k** (peak fear; std ~6) → climbed back through 0 by ≈1.21M → currently **+1 to +3** (prey actively reward proximity to predators!) for most of the 1.3M-4.9M window. Poly prey at 4.7M shows the same lack-of-fear pattern: `act*pred` (flee response) is only **+1.25** while `act*prey` (herd response) dominates at **+4.07**, AND `pred^2` evolved from +0.50 (3.0M) to **+1.36** (4.7M) — predator-magnitude is rewarding, not aversive. The "panic-feed" term (`eat*pred`) was at +2.23 at 3.2M and crashed to +0.40 by 4.5M.

(For comparison: predator w_pred drifted *much* harder, hitting **-9.24** mean at 1.81M with std 19.8 — extreme variance during the fearful-loner lineage's birth — before settling at -1.4 average over the rest of that regime. That was a predator-side bottleneck/lineage event, not prey fear; the prey-fear arc is more modest.)
